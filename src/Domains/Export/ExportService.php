...
final class ExportService
{
    public function manufacturingPackageCsv(int $tenantId, int $packageId): array
    {
        ...
        $cols = ['SL', 'DESCRIPTION', 'FINISH_L', 'FINISH_W', 'CUT_L', 'CUT_W', 'THICKNESS', 'QTY', 'MATERIAL', 'COLOUR', 'E1', 'E2', 'E3', 'E4', 'NOTE'];
        $lines = [implode(',', $cols)];
        foreach ($cut['items'] as $i => $row) {
            $lines[] = implode(',', [
                $i + 1,
                $this->csv($row['description'] ?? ''),
        ...
        $name = 'cutlist-pkg-' . $packageId . '-' . time() . '.csv';
        $path = $dir . '/' . $name;
        file_put_contents($path, $csv);
        Audit::record('EXPORT', 'cutlist_csv', $packageId, null, ['file' => $name]);
        return [
            'filename' => $name,
            'path' => 'storage/exports/' . $name,
            'mime' => 'text/csv',
            'content' => $csv,
        ];
    }
