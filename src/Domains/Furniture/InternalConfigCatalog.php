<?php

declare(strict_types=1);

namespace Fmos\Domains\Furniture;

/**
 * Named internal configurations that compile to layout section patches.
 *
 * Phase 1 maps to existing section types only (HANGING/SHELVES/DRAWERS/OPEN/MIRROR).
 * Specialty kitchen/wardrobe configs generate real LayoutEngine panels/hardware when implemented.
 *
 * Eligibility is overlaid from config/fmosv2 extracts (FMOSV2 configuration-rules + sections)
 * via FmosV2RulesBridge when available.
 */
final class InternalConfigCatalog
{
    public const SECTION_TYPES = ['HANGING', 'SHELVES', 'DRAWERS', 'OPEN', 'MIRROR'];

    /**
     * @return array<string, array<string,mixed>>
     */
    public static function all(): array
    {
        return self::applyV2Eligibility(self::definitions());
    }

    /**
     * @return array<string, array<string,mixed>>
     */
    private static function definitions(): array
    {
        return [
            // —— Wardrobe / shared storage ——
            'CFG_HANGING' => [
                'id' => 'CFG_HANGING',
                'name' => 'Hanging section',
                'category' => 'STORAGE',
                'description' => 'Hanging rod + cleat (LayoutEngine HANGING)',
                'maps_to_section_type' => 'HANGING',
                'implemented' => true,
                'tier' => 'recommended',
                'eligibility' => [
                    'min_height_mm' => 900,
                    'min_width_mm' => 300,
                    'min_depth_mm' => 400,
                    'rule_source' => 'business_default',
                ],
                'section_defaults' => [
                    'type' => 'HANGING',
                    'height_mm' => 1100,
                    'hanging_style' => 'standard',
                    'label' => 'Hanging',
                ],
                'incompatible_with' => [],
                'kitchen_preset' => null,
                'fmosv2_section_ids' => ['hanging_full', 'hanging_double'],
            ],
            'CFG_HANGING_LONG' => [
                'id' => 'CFG_HANGING_LONG',
                'name' => 'Long hanging',
                'category' => 'WARDROBE',
                'description' => 'Full-height hanging with top shelf (FMOSV2 hanging_full)',
                'maps_to_section_type' => 'HANGING',
                'implemented' => true,
                'tier' => 'optional',
                'eligibility' => [
                    'min_height_mm' => 1400,
                    'min_width_mm' => 450,
                    'min_depth_mm' => 550,
                    'rule_source' => 'business_default',
                ],
                'section_defaults' => [
                    'type' => 'HANGING',
                    'height_mm' => 1600,
                    'hanging_style' => 'long',
                    'top_shelf_count' => 1,
                    'label' => 'Long hanging',
                ],
                'incompatible_with' => [],
                'kitchen_preset' => null,
                'fmosv2_section_ids' => ['hanging_full'],
            ],
            'CFG_HANGING_DOUBLE' => [
                'id' => 'CFG_HANGING_DOUBLE',
                'name' => 'Double / short hanging',
                'category' => 'WARDROBE',
                'description' => 'Two hanging levels with mid shelf (FMOSV2 hanging_double)',
                'maps_to_section_type' => 'HANGING',
                'implemented' => true,
                'tier' => 'optional',
                'eligibility' => [
                    'min_height_mm' => 1400,
                    'min_width_mm' => 450,
                    'min_depth_mm' => 550,
                    'rule_source' => 'business_default',
                ],
                'section_defaults' => [
                    'type' => 'HANGING',
                    'height_mm' => 1400,
                    'hanging_style' => 'double',
                    'label' => 'Double hanging',
                ],
                'incompatible_with' => [],
                'kitchen_preset' => null,
                'fmosv2_section_ids' => ['hanging_double'],
            ],
            'CFG_SHELVES' => [
                'id' => 'CFG_SHELVES',
                'name' => 'Adjustable shelves',
                'category' => 'STORAGE',
                'description' => 'Fixed/adjustable shelf pack (existing SHELVES)',
                'maps_to_section_type' => 'SHELVES',
                'implemented' => true,
                'tier' => 'recommended',
                'eligibility' => [
                    'min_height_mm' => 200,
                    'min_width_mm' => 250,
                    'min_depth_mm' => 250,
                    'rule_source' => 'business_default',
                ],
                'section_defaults' => [
                    'type' => 'SHELVES',
                    'height_mm' => null,
                    'shelf_count' => 3,
                    'label' => 'Shelves',
                ],
                'incompatible_with' => [],
                'kitchen_preset' => 'shelf',
            ],
            'CFG_DRAWERS' => [
                'id' => 'CFG_DRAWERS',
                'name' => 'Drawer pack',
                'category' => 'STORAGE',
                'description' => 'Drawer fronts/boxes + slides (existing DRAWERS)',
                'maps_to_section_type' => 'DRAWERS',
                'implemented' => true,
                'tier' => 'optional',
                'eligibility' => [
                    'min_height_mm' => 350,
                    'min_width_mm' => 300,
                    'min_depth_mm' => 350,
                    'rule_source' => 'business_default',
                ],
                'section_defaults' => [
                    'type' => 'DRAWERS',
                    'height_mm' => 750,
                    'drawer_count' => 3,
                    'drawer_height_mm' => 200,
                    'label' => 'Drawers',
                ],
                'layout_overrides' => [],
                'incompatible_with' => [],
                'kitchen_preset' => 'drawers',
            ],
            'CFG_OPEN' => [
                'id' => 'CFG_OPEN',
                'name' => 'Open niche',
                'category' => 'STORAGE',
                'description' => 'Open niche with liners (existing OPEN)',
                'maps_to_section_type' => 'OPEN',
                'implemented' => true,
                'tier' => 'optional',
                'eligibility' => [
                    'min_height_mm' => 150,
                    'min_width_mm' => 250,
                    'min_depth_mm' => 250,
                    'rule_source' => 'business_default',
                ],
                'section_defaults' => [
                    'type' => 'OPEN',
                    'height_mm' => null,
                    'label' => 'Open',
                ],
                'incompatible_with' => [],
                'kitchen_preset' => 'open',
            ],
            'CFG_MIRROR' => [
                'id' => 'CFG_MIRROR',
                'name' => 'Internal mirror',
                'category' => 'ACCESSORY',
                'description' => 'Mirror glass in niche (existing MIRROR; does not force unit EXPO)',
                'maps_to_section_type' => 'MIRROR',
                'implemented' => true,
                'tier' => 'optional',
                'eligibility' => [
                    'min_height_mm' => 400,
                    'min_width_mm' => 300,
                    'min_depth_mm' => 300,
                    'rule_source' => 'business_default',
                ],
                'section_defaults' => [
                    'type' => 'MIRROR',
                    'height_mm' => null,
                    'mirror_margin_mm' => 80,
                    'label' => 'Mirror',
                ],
                'incompatible_with' => [],
                'kitchen_preset' => null,
            ],

            // —— Kitchen-oriented aliases (same section types) ——
            'CFG_KB_SHELF' => [
                'id' => 'CFG_KB_SHELF',
                'name' => 'Kitchen standard shelf',
                'category' => 'KITCHEN',
                'description' => 'Single adjustable shelf for kitchen base',
                'maps_to_section_type' => 'SHELVES',
                'implemented' => true,
                'tier' => 'recommended',
                'eligibility' => [
                    'min_height_mm' => 400,
                    'min_width_mm' => 300,
                    'min_depth_mm' => 400,
                    'rule_source' => 'business_default',
                ],
                'section_defaults' => [
                    'type' => 'SHELVES',
                    'height_mm' => null,
                    'shelf_count' => 1,
                    'label' => 'Shelf',
                ],
                'replace_bay' => true,
                'incompatible_with' => ['CFG_KB_DRAWERS', 'CFG_KB_SINK'],
                'kitchen_preset' => 'shelf',
            ],
            'CFG_KB_DRAWERS' => [
                'id' => 'CFG_KB_DRAWERS',
                'name' => 'Kitchen drawer base',
                'category' => 'KITCHEN',
                'description' => 'Full drawer pack; sets door_type NONE',
                'maps_to_section_type' => 'DRAWERS',
                'implemented' => true,
                'tier' => 'optional',
                'eligibility' => [
                    'min_height_mm' => 500,
                    'min_width_mm' => 400,
                    'min_depth_mm' => 450,
                    'rule_source' => 'business_default',
                ],
                'section_defaults' => [
                    'type' => 'DRAWERS',
                    'height_mm' => null,
                    'drawer_count' => 3,
                    'drawer_height_mm' => 180,
                    'label' => 'Drawers',
                ],
                'layout_overrides' => [
                    'door_type' => 'NONE',
                ],
                'replace_bay' => true,
                'incompatible_with' => ['CFG_KB_SHELF', 'CFG_KB_SINK'],
                'kitchen_preset' => 'drawers',
            ],
            'CFG_KB_SINK' => [
                'id' => 'CFG_KB_SINK',
                'name' => 'Sink / plumbing clearance',
                'category' => 'KITCHEN',
                'description' => 'Open niche for plumbing (Phase 1 = OPEN; specialty sink carcass later)',
                'maps_to_section_type' => 'OPEN',
                'implemented' => true,
                'tier' => 'optional',
                'eligibility' => [
                    'min_height_mm' => 500,
                    'min_width_mm' => 450,
                    'min_depth_mm' => 450,
                    'rule_source' => 'business_default',
                ],
                'section_defaults' => [
                    'type' => 'OPEN',
                    'height_mm' => null,
                    'label' => 'Plumbing',
                ],
                'replace_bay' => true,
                'incompatible_with' => ['CFG_KB_SHELF', 'CFG_KB_DRAWERS', 'CFG_KB_PLATE_TRAY'],
                'kitchen_preset' => 'sink',
            ],

            // —— Specialty pull-outs / racks (real LayoutEngine panels + hardware) ——
            'CFG_KB_PLATE_TRAY' => [
                'id' => 'CFG_KB_PLATE_TRAY',
                'name' => 'Plate tray',
                'category' => 'KITCHEN',
                'description' => 'Pull-out plate trays (tray boards + runners; FMOSV2 pull_out)',
                'maps_to_section_type' => 'SHELVES',
                'implemented' => true,
                'tier' => 'optional',
                'eligibility' => [
                    'min_height_mm' => 500,
                    'min_width_mm' => 500,
                    'min_depth_mm' => 500,
                    'rule_source' => 'business_default',
                ],
                'section_defaults' => [
                    'type' => 'SHELVES',
                    'height_mm' => null,
                    'shelf_count' => 5,
                    'shelf_style' => 'plate_tray',
                    'label' => 'Plate trays',
                ],
                'layout_overrides' => [
                    'door_type' => 'NONE',
                ],
                'replace_bay' => true,
                'incompatible_with' => ['CFG_KB_SINK'],
                'kitchen_preset' => null,
                'fmosv2_section_ids' => ['pull_out'],
            ],
            'CFG_KB_CUTLERY' => [
                'id' => 'CFG_KB_CUTLERY',
                'name' => 'Cutlery drawer',
                'category' => 'KITCHEN',
                'description' => 'Drawer pack + cutlery organizer hardware (BOM accessory, not a board)',
                'maps_to_section_type' => 'DRAWERS',
                'implemented' => true,
                'tier' => 'optional',
                'eligibility' => [
                    'min_height_mm' => 150,
                    'min_width_mm' => 400,
                    'min_depth_mm' => 450,
                    'rule_source' => 'business_default',
                ],
                'section_defaults' => [
                    'type' => 'DRAWERS',
                    'height_mm' => null,
                    'drawer_count' => 1,
                    'drawer_height_mm' => 120,
                    'cutlery_organizer' => true,
                    'label' => 'Cutlery',
                ],
                'layout_overrides' => [
                    'door_type' => 'NONE',
                ],
                'replace_bay' => true,
                'incompatible_with' => ['CFG_KB_SINK', 'CFG_KB_SHELF'],
                'kitchen_preset' => null,
                'fmosv2_section_ids' => ['cutlery_organizer'],
            ],
            'CFG_KB_BOTTLE' => [
                'id' => 'CFG_KB_BOTTLE',
                'name' => 'Bottle pull-out',
                'category' => 'KITCHEN',
                'description' => 'Narrow bottle/spice pull-out (rack shelves + pull-out hardware)',
                'maps_to_section_type' => 'SHELVES',
                'implemented' => true,
                'tier' => 'optional',
                'eligibility' => [
                    'min_height_mm' => 500,
                    'min_width_mm' => 150,
                    'max_width_mm' => 300,
                    'min_depth_mm' => 450,
                    'rule_source' => 'business_default',
                ],
                'section_defaults' => [
                    'type' => 'SHELVES',
                    'height_mm' => null,
                    'shelf_count' => 3,
                    'shelf_style' => 'bottle',
                    'label' => 'Bottle pull-out',
                ],
                'layout_overrides' => [
                    'door_type' => 'NONE',
                ],
                'replace_bay' => true,
                'incompatible_with' => [],
                'kitchen_preset' => null,
                'fmosv2_section_ids' => ['bottle_pullout', 'pull_out'],
            ],
            'CFG_KB_WASTE' => [
                'id' => 'CFG_KB_WASTE',
                'name' => 'Waste bin module',
                'category' => 'KITCHEN',
                'description' => 'Waste pull-out bay (niche liners + bin hardware + runners)',
                'maps_to_section_type' => 'OPEN',
                'implemented' => true,
                'tier' => 'optional',
                'eligibility' => [
                    'min_height_mm' => 500,
                    'min_width_mm' => 400,
                    'min_depth_mm' => 450,
                    'rule_source' => 'business_default',
                ],
                'section_defaults' => [
                    'type' => 'OPEN',
                    'height_mm' => null,
                    'waste_bin' => true,
                    'label' => 'Waste bin',
                ],
                'layout_overrides' => [
                    'door_type' => 'NONE',
                ],
                'replace_bay' => true,
                'incompatible_with' => [],
                'kitchen_preset' => null,
            ],
            'CFG_SHOE' => [
                'id' => 'CFG_SHOE',
                'name' => 'Shoe storage',
                'category' => 'WARDROBE',
                'description' => 'Shoe shelf pack (real SHELF_SHOE panels + supports)',
                'maps_to_section_type' => 'SHELVES',
                'implemented' => true,
                'tier' => 'optional',
                'eligibility' => [
                    'min_height_mm' => 400,
                    'min_width_mm' => 400,
                    'min_depth_mm' => 350,
                    'rule_source' => 'business_default',
                ],
                'section_defaults' => [
                    'type' => 'SHELVES',
                    'height_mm' => 600,
                    'shelf_count' => 3,
                    'shelf_style' => 'shoe',
                    'label' => 'Shoe shelves',
                ],
                'incompatible_with' => [],
                'kitchen_preset' => null,
                'fmosv2_section_ids' => ['shoe_rack'],
            ],
            'CFG_TROUSER' => [
                'id' => 'CFG_TROUSER',
                'name' => 'Trouser pull-out',
                'category' => 'WARDROBE',
                'description' => 'Trouser rack pull-out (rack hardware + runners; FMOSV2 trouser_pull_out)',
                'maps_to_section_type' => 'OPEN',
                'implemented' => true,
                'tier' => 'optional',
                'eligibility' => [
                    'min_width_mm' => 450,
                    'min_height_mm' => 400,
                    'min_depth_mm' => 450,
                    'rule_source' => 'business_default',
                ],
                'section_defaults' => [
                    'type' => 'OPEN',
                    'height_mm' => 600,
                    'trouser_rack' => true,
                    'label' => 'Trouser pull-out',
                ],
                'incompatible_with' => [],
                'kitchen_preset' => null,
                'fmosv2_section_ids' => ['trouser_pull_out'],
            ],
            'CFG_WICKER' => [
                'id' => 'CFG_WICKER',
                'name' => 'Wicker basket',
                'category' => 'WARDROBE',
                'description' => 'Wicker/laundry baskets on runners (fronts + basket hardware)',
                'maps_to_section_type' => 'DRAWERS',
                'implemented' => true,
                'tier' => 'optional',
                'eligibility' => [
                    'min_width_mm' => 350,
                    'min_height_mm' => 300,
                    'min_depth_mm' => 400,
                    'rule_source' => 'business_default',
                ],
                'section_defaults' => [
                    'type' => 'DRAWERS',
                    'height_mm' => 600,
                    'drawer_count' => 3,
                    'drawer_height_mm' => 200,
                    'wicker_basket' => true,
                    'label' => 'Wicker baskets',
                ],
                'layout_overrides' => [
                    'door_type' => 'NONE',
                ],
                'replace_bay' => false,
                'incompatible_with' => [],
                'kitchen_preset' => null,
                'fmosv2_section_ids' => ['wicker_basket'],
            ],
            'CFG_KB_HOB' => [
                'id' => 'CFG_KB_HOB',
                'name' => 'Hob module',
                'category' => 'KITCHEN',
                'description' => 'Hob/cooktop bay (open service void + niche liners; no shelves)',
                'maps_to_section_type' => 'OPEN',
                'implemented' => true,
                'tier' => 'optional',
                'eligibility' => [
                    'min_width_mm' => 600,
                    'min_height_mm' => 720,
                    'min_depth_mm' => 560,
                    'rule_source' => 'business_default',
                ],
                'section_defaults' => [
                    'type' => 'OPEN',
                    'height_mm' => null,
                    'hob_bay' => true,
                    'label' => 'Hob bay',
                ],
                'layout_overrides' => [
                    'door_type' => 'NONE',
                ],
                'replace_bay' => true,
                'incompatible_with' => ['CFG_KB_SHELF', 'CFG_KB_DRAWERS'],
                'kitchen_preset' => null,
                'fmosv2_section_ids' => ['hob_bay'],
            ],
        ];
    }

    /**
     * Overlay eligibility from FMOSV2 extracts when present.
     *
     * @param array<string, array<string,mixed>> $defs
     * @return array<string, array<string,mixed>>
     */
    private static function applyV2Eligibility(array $defs): array
    {
        if (!FmosV2RulesBridge::available()) {
            return $defs;
        }
        foreach ($defs as $id => &$cfg) {
            $overlay = FmosV2RulesBridge::eligibilityForConfig($id);
            if ($overlay === []) {
                continue;
            }
            $cfg['eligibility'] = array_merge($cfg['eligibility'] ?? [], $overlay);
        }
        unset($cfg);
        return $defs;
    }

    /** @return array<string,mixed>|null */
    public static function get(string $id): ?array
    {
        $all = self::all();
        return $all[$id] ?? null;
    }

    /**
     * Layout presets formerly hardcoded in furniture.js — served from catalog.
     *
     * @return list<array{id:string,label:string,category:string,config_ids:list<string>,layout:array<string,mixed>}>
     */
    public static function layoutPresets(?string $category = null): array
    {
        $common = static fn (string $doorType = 'HINGED') => [
            'partition_thickness_mm' => 18,
            'door_type' => $doorType,
            'loft' => ['enabled' => false, 'height_mm' => 600, 'shelf_count' => 1],
        ];

        $presets = [
            [
                'id' => 'hang-draw-shelf',
                'label' => 'Hang + drawers + shelves',
                'category' => 'WARDROBE',
                'config_ids' => ['CFG_HANGING', 'CFG_DRAWERS', 'CFG_SHELVES'],
                'layout' => $common() + [
                    'plinth_height_mm' => 110,
                    'bays' => [
                        [
                            'id' => 'bay-1',
                            'label' => 'Hang/Draw',
                            'width_mm' => null,
                            'sections' => [
                                ['type' => 'HANGING', 'height_mm' => 1100, 'label' => 'Hanging'],
                                ['type' => 'DRAWERS', 'height_mm' => 750, 'drawer_count' => 3, 'drawer_height_mm' => 200, 'label' => 'Drawers'],
                                ['type' => 'SHELVES', 'height_mm' => null, 'shelf_count' => 1, 'label' => 'Bottom'],
                            ],
                        ],
                        [
                            'id' => 'bay-2',
                            'label' => 'Shelves',
                            'width_mm' => null,
                            'sections' => [
                                ['type' => 'SHELVES', 'height_mm' => null, 'shelf_count' => 5, 'label' => 'Shelves'],
                            ],
                        ],
                    ],
                ],
            ],
            [
                'id' => 'full-hang',
                'label' => 'Full hanging',
                'category' => 'WARDROBE',
                'config_ids' => ['CFG_HANGING'],
                'layout' => $common() + [
                    'plinth_height_mm' => 110,
                    'bays' => [[
                        'id' => 'bay-1',
                        'label' => 'Hanging',
                        'width_mm' => null,
                        'sections' => [['type' => 'HANGING', 'height_mm' => null, 'label' => 'Hanging']],
                    ]],
                ],
            ],
            [
                'id' => 'with-loft',
                'label' => '2 bay + loft',
                'category' => 'WARDROBE',
                'config_ids' => ['CFG_HANGING', 'CFG_DRAWERS', 'CFG_SHELVES'],
                'layout' => [
                    'partition_thickness_mm' => 18,
                    'door_type' => 'HINGED',
                    'plinth_height_mm' => 110,
                    'loft' => ['enabled' => true, 'height_mm' => 600, 'shelf_count' => 1],
                    'bays' => [
                        [
                            'id' => 'bay-1',
                            'label' => 'Left',
                            'width_mm' => null,
                            'sections' => [
                                ['type' => 'HANGING', 'height_mm' => 1100, 'label' => 'Hanging'],
                                ['type' => 'DRAWERS', 'height_mm' => null, 'drawer_count' => 3, 'drawer_height_mm' => 200, 'label' => 'Drawers'],
                            ],
                        ],
                        [
                            'id' => 'bay-2',
                            'label' => 'Right',
                            'width_mm' => null,
                            'sections' => [
                                ['type' => 'SHELVES', 'height_mm' => null, 'shelf_count' => 5, 'label' => 'Shelves'],
                            ],
                        ],
                    ],
                ],
            ],
            [
                'id' => 'tv-3bay',
                'label' => 'Open centre + side storage',
                'category' => 'TV_UNIT',
                'config_ids' => ['CFG_OPEN', 'CFG_DRAWERS', 'CFG_SHELVES'],
                'layout' => $common() + [
                    'plinth_height_mm' => 80,
                    'bays' => [
                        [
                            'id' => 'bay-1',
                            'label' => 'Left',
                            'width_mm' => null,
                            'sections' => [
                                ['type' => 'OPEN', 'height_mm' => 220, 'label' => 'Niche'],
                                ['type' => 'DRAWERS', 'height_mm' => null, 'drawer_count' => 2, 'drawer_height_mm' => 180, 'label' => 'Drawers'],
                            ],
                        ],
                        [
                            'id' => 'bay-2',
                            'label' => 'TV niche',
                            'width_mm' => null,
                            'sections' => [['type' => 'OPEN', 'height_mm' => null, 'label' => 'Open']],
                        ],
                        [
                            'id' => 'bay-3',
                            'label' => 'Right',
                            'width_mm' => null,
                            'sections' => [
                                ['type' => 'OPEN', 'height_mm' => 220, 'label' => 'Niche'],
                                ['type' => 'SHELVES', 'shelf_count' => 1, 'height_mm' => null, 'label' => 'Shelf'],
                            ],
                        ],
                    ],
                ],
            ],
            [
                'id' => 'tv-drawers',
                'label' => 'All drawers',
                'category' => 'TV_UNIT',
                'config_ids' => ['CFG_DRAWERS'],
                'layout' => $common() + [
                    'plinth_height_mm' => 80,
                    'bays' => [[
                        'id' => 'bay-1',
                        'label' => 'Drawers',
                        'width_mm' => null,
                        'sections' => [[
                            'type' => 'DRAWERS',
                            'height_mm' => null,
                            'drawer_count' => 3,
                            'drawer_height_mm' => 160,
                            'label' => 'Drawers',
                        ]],
                    ]],
                ],
            ],
            [
                'id' => 'kit-shelf',
                'label' => 'Single shelf',
                'category' => 'KITCHEN',
                'config_ids' => ['CFG_KB_SHELF'],
                'layout' => $common() + [
                    'plinth_height_mm' => 100,
                    'bays' => [[
                        'id' => 'bay-1',
                        'label' => 'Cabinet',
                        'width_mm' => null,
                        'sections' => [['type' => 'SHELVES', 'shelf_count' => 1, 'height_mm' => null, 'label' => 'Shelf']],
                    ]],
                ],
            ],
            [
                'id' => 'kit-drawers',
                'label' => 'Drawer pack',
                'category' => 'KITCHEN',
                'config_ids' => ['CFG_KB_DRAWERS'],
                'layout' => [
                    'partition_thickness_mm' => 18,
                    'door_type' => 'NONE',
                    'plinth_height_mm' => 100,
                    'loft' => ['enabled' => false, 'height_mm' => 0, 'shelf_count' => 0],
                    'bays' => [[
                        'id' => 'bay-1',
                        'label' => 'Drawers',
                        'width_mm' => null,
                        'sections' => [[
                            'type' => 'DRAWERS',
                            'drawer_count' => 3,
                            'drawer_height_mm' => 150,
                            'height_mm' => null,
                            'label' => 'Drawers',
                        ]],
                    ]],
                ],
            ],
            [
                'id' => 'kit-pantry',
                'label' => 'Pantry shelves',
                'category' => 'KITCHEN',
                'config_ids' => ['CFG_SHELVES'],
                'layout' => $common() + [
                    'plinth_height_mm' => 100,
                    'bays' => [[
                        'id' => 'bay-1',
                        'label' => 'Pantry',
                        'width_mm' => null,
                        'sections' => [['type' => 'SHELVES', 'shelf_count' => 5, 'height_mm' => null, 'label' => 'Shelves']],
                    ]],
                ],
            ],
            [
                'id' => 'kit-sink',
                'label' => 'Sink / plumbing',
                'category' => 'KITCHEN',
                'config_ids' => ['CFG_KB_SINK'],
                'layout' => $common() + [
                    'plinth_height_mm' => 100,
                    'bays' => [[
                        'id' => 'bay-1',
                        'label' => 'Sink',
                        'width_mm' => null,
                        'sections' => [['type' => 'OPEN', 'height_mm' => null, 'label' => 'Plumbing']],
                    ]],
                ],
            ],
            [
                'id' => 'chest',
                'label' => '4 drawers',
                'category' => 'STORAGE',
                'config_ids' => ['CFG_DRAWERS'],
                'layout' => [
                    'partition_thickness_mm' => 18,
                    'door_type' => 'NONE',
                    'plinth_height_mm' => 80,
                    'loft' => ['enabled' => false, 'height_mm' => 0, 'shelf_count' => 0],
                    'bays' => [[
                        'id' => 'bay-1',
                        'label' => 'Chest',
                        'width_mm' => null,
                        'sections' => [[
                            'type' => 'DRAWERS',
                            'drawer_count' => 4,
                            'drawer_height_mm' => 180,
                            'height_mm' => null,
                            'label' => 'Drawers',
                        ]],
                    ]],
                ],
            ],
            [
                'id' => 'books',
                'label' => 'Open shelves',
                'category' => 'STORAGE',
                'config_ids' => ['CFG_SHELVES'],
                'layout' => [
                    'partition_thickness_mm' => 18,
                    'door_type' => 'NONE',
                    'plinth_height_mm' => 0,
                    'loft' => ['enabled' => false, 'height_mm' => 0, 'shelf_count' => 0],
                    'bays' => [[
                        'id' => 'bay-1',
                        'label' => 'Shelves',
                        'width_mm' => null,
                        'sections' => [['type' => 'SHELVES', 'shelf_count' => 5, 'height_mm' => null, 'label' => 'Shelves']],
                    ]],
                ],
            ],
            [
                'id' => 'vanity-draw',
                'label' => '2 drawers',
                'category' => 'BATHROOM',
                'config_ids' => ['CFG_DRAWERS'],
                'layout' => $common() + [
                    'plinth_height_mm' => 80,
                    'bays' => [[
                        'id' => 'bay-1',
                        'label' => 'Vanity',
                        'width_mm' => null,
                        'sections' => [[
                            'type' => 'DRAWERS',
                            'drawer_count' => 2,
                            'drawer_height_mm' => 180,
                            'height_mm' => null,
                            'label' => 'Drawers',
                        ]],
                    ]],
                ],
            ],
        ];

        $presets = array_merge($presets, self::wardrobePresetsFromV2());

        if ($category === null || $category === '') {
            return $presets;
        }
        $cat = strtoupper($category);
        return array_values(array_filter(
            $presets,
            static fn ($p) => strtoupper((string) $p['category']) === $cat
        ));
    }

    /**
     * Compile FMOSV2 WardrobePresets.json tiers into layout presets.
     *
     * @return list<array{id:string,label:string,category:string,config_ids:list<string>,layout:array<string,mixed>}>
     */
    public static function wardrobePresetsFromV2(): array
    {
        $path = FmosV2RulesBridge::configDir() . '/wardrobe-presets.json';
        if (!is_file($path)) {
            return [];
        }
        $raw = file_get_contents($path);
        if ($raw === false) {
            return [];
        }
        if (str_starts_with($raw, "\xEF\xBB\xBF")) {
            $raw = substr($raw, 3);
        }
        $data = json_decode($raw, true);
        if (!is_array($data)) {
            return [];
        }
        $out = [];
        foreach ($data['presets'] ?? [] as $preset) {
            if (!is_array($preset)) {
                continue;
            }
            $compiled = self::compileV2WardrobePreset($preset);
            if ($compiled !== null) {
                $out[] = $compiled;
            }
        }
        return $out;
    }

    /**
     * @param array<string,mixed> $preset
     * @return array{id:string,label:string,category:string,config_ids:list<string>,layout:array<string,mixed>}|null
     */
    private static function compileV2WardrobePreset(array $preset): ?array
    {
        $tiers = $preset['tiers'] ?? [];
        if (!is_array($tiers) || $tiers === []) {
            return null;
        }
        usort($tiers, static fn ($a, $b) => ((int) ($a['tier_index'] ?? 0)) <=> ((int) ($b['tier_index'] ?? 0)));

        $sections = [];
        $configIds = [];
        foreach ($tiers as $tier) {
            foreach ($tier['sections'] ?? [] as $sec) {
                $mapped = self::mapV2PresetSection($sec);
                if ($mapped === null) {
                    continue;
                }
                $sections[] = $mapped['section'];
                foreach ($mapped['config_ids'] as $cid) {
                    $configIds[$cid] = true;
                }
            }
        }
        if ($sections === []) {
            return null;
        }

        return [
            'id' => (string) ($preset['id'] ?? ('v2-' . md5((string) ($preset['name'] ?? 'preset')))),
            'label' => (string) ($preset['name'] ?? 'Wardrobe preset'),
            'category' => 'WARDROBE',
            'config_ids' => array_keys($configIds),
            'layout' => [
                'plinth_height_mm' => 110,
                'partition_thickness_mm' => 18,
                'door_type' => 'HINGED',
                'loft' => ['enabled' => false, 'height_mm' => 0, 'shelf_count' => 0],
                'bays' => [[
                    'id' => 'bay-1',
                    'label' => 'Main',
                    'width_mm' => null,
                    'sections' => $sections,
                ]],
            ],
        ];
    }

    /**
     * @param array<string,mixed> $sec
     * @return array{section:array<string,mixed>,config_ids:list<string>}|null
     */
    private static function mapV2PresetSection(array $sec): ?array
    {
        $type = strtolower((string) ($sec['section_type'] ?? ''));
        $label = (string) ($sec['label'] ?? ucfirst($type));
        $height = isset($sec['height_mm']) ? (float) $sec['height_mm'] : null;
        return match ($type) {
            'hanging' => [
                'section' => [
                    'type' => 'HANGING',
                    'height_mm' => $height ?? 1100,
                    'hanging_style' => ($height !== null && $height >= 1400) ? 'long' : 'standard',
                    'label' => $label,
                ],
                'config_ids' => ($height !== null && $height >= 1400)
                    ? ['CFG_HANGING_LONG', 'CFG_HANGING']
                    : ['CFG_HANGING'],
            ],
            'shelf', 'shelves' => [
                'section' => [
                    'type' => 'SHELVES',
                    'height_mm' => $height,
                    'shelf_count' => max(1, (int) ($sec['shelf_count'] ?? 3)),
                    'label' => $label,
                ],
                'config_ids' => ['CFG_SHELVES'],
            ],
            'drawer', 'drawers' => [
                'section' => [
                    'type' => 'DRAWERS',
                    'height_mm' => $height,
                    'drawer_count' => max(1, (int) ($sec['drawer_count'] ?? 3)),
                    'drawer_height_mm' => (float) ($sec['drawer']['depth_mm'] ?? 200) > 400
                        ? 180
                        : (float) ($sec['drawer_height_mm'] ?? 180),
                    'label' => $label,
                ],
                'config_ids' => ['CFG_DRAWERS'],
            ],
            'shoe', 'shoe_rack' => [
                'section' => [
                    'type' => 'SHELVES',
                    'height_mm' => $height ?? 600,
                    'shelf_count' => 3,
                    'shelf_style' => 'shoe',
                    'label' => $label !== '' ? $label : 'Shoe shelves',
                ],
                'config_ids' => ['CFG_SHOE'],
            ],
            default => null,
        };
    }

    /** Map kitchen composition spawn preset → config id. */
    public static function configIdForKitchenPreset(string $preset): string
    {
        return match (strtolower($preset)) {
            'drawers' => 'CFG_KB_DRAWERS',
            'sink', 'open' => 'CFG_KB_SINK',
            default => 'CFG_KB_SHELF',
        };
    }
}
